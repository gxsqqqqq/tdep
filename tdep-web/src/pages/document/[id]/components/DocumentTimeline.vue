<template>
  <div class="document-timeline-card">
    <div class="timeline-card-header">
      <div class="flex items-center gap-2">
        <el-icon :size="20" color="#409EFF"><Document /></el-icon>
        <h3 class="text-base font-bold text-gray-800">文书生命周期</h3>
      </div>
    ...
    <transition name="slide-fade" mode="out-in">
      <div :key="selectedStepIndex" class="step-detail-panel">
        <div class="detail-panel-inner">
          ...
          <div v-if="selectedStep?.allowedTransitions && selectedStep.allowedTransitions.length > 0" class="allowed-transitions mt-3 pt-3 border-t border-gray-100">
            <p class="text-xs text-gray-500 mb-2 font-medium">可执行操作：</p>
            <div class="flex flex-wrap gap-2">
              <el-tag
                v-for="transition in selectedStep.allowedTransitions"
                :key="transition"
                size="small"
                type="info"
                round
...
const timelineSteps = computed(() => getDocumentStatusTimeline())
...
function getTransitionLabel(transition: DocumentTransition): string {
  const labelMap: Record<DocumentTransition, string> = {
    [DocumentTransition.GENERATE]: '生成文书',
    [DocumentTransition.REVIEW]: '提交审核',
    [DocumentTransition.SIGN]: '签署确认',
    [DocumentTransition.ARCHIVE]: '归档文书',
  }
  return labelMap[transition] || transition
}
</script>
